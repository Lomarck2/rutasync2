...
var busStations, Noticias;
var ubicacionLatLng, ubicacionAntLatLng;
...
function mostrarUbicacionUsuario(bandera) {
  ...
  if (navigator.geolocation) {
    navigator.geolocation.getCurrentPosition((position) => {
      ...
      if(bandera==1){
        initMap(lat, lng);
        ubicacionAntLatLng = {lat:lat, lng:lng};
      }
      ...
      geocoder.geocode({ location: latlng }, (results, status) => {
        if (status === 'OK' && results[0]) {
          ...
          if(separacion(ubicacionAntLatLng,ubicacionLatLng,400) || bandera==0){
            var markerUbi = new google.maps.marker.AdvancedMarkerElement({
                position: latlng,
                map: map,
                title: "Tu ubicación"
            });
            listaMarcadores.push(markerUbi);
            ubicacionAntLatLng=ubicacionLatLng;
            console.log("SI");
          }
        }
      });
...
function abrirChat() {
  const mensaje = prompt("Reporta de un Evento en la ruta:");
...
function dibuMapa(){
    ...
    for(i=0; i<tam; i++){
        ...
        marker = makeMarkerIcono(newLatLng,0,'Parada Origen',listaParadasEnRuta[i],i);
...
function mostrarEventos(ruta){
  var tam2 = Noticias[ruta]['Noticias'].length;
  var notis = Noticias[ruta]['Noticias']; 
  var caja = document.getElementById('notificacionesBox');   
  if(tam2 > 0){
    caja.innerHTML = "";
    for(var i=0; i<tam2; i++){
      var fecha1 = new Date(notis[i]["fecha"]);
      var fecha2 = new Date();
      var diff = new Date(fecha2.getTime() - fecha1.getTime());
      var mesDif = diff.getUTCMonth();
      if(mesDif <= 3)
        caja.innerHTML += (i+1) + ". " + notis[i]["evento"] + "<br>";      
    }
  }
  else
    caja.innerHTML = "Sin noticias";
}
...
function mostrarParadaEnRuta(paradas,paraO,paraD){    
  ...
  tiempoViaje(paradas,paraO,paraD)
  directionsService = new google.maps.DirectionsService();
...
function actualizarUbicacion(){
  ...
  if (navigator.geolocation) {
    navigator.geolocation.getCurrentPosition((position) => {
      ...
      ubicacionLatLng = {lat:lat, lng:lng};
      //origenLatLng = {lat:lat, lng:lng}; 
      if(separacion(ubicacionAntLatLng,ubicacionLatLng,400)){
        /* var markerUbi = new google.maps.marker.AdvancedMarkerElement({
          position: latlng,
          map: map,
          title: "Tu ubicación"
        }); */               
        if(listaMarcadores.length > 1){
          var listAuxiliar = [];
          for(var i=0; i>listaMarcadores.length; i++){
            listAuxiliar.push(getUbicacion(listaMarcadores[i]);
            listaMarcadores.setMap(null);          
          }
          listaMarcadores = []
          ...
          markerNuevo = makeMarker2(ubicacionLatLng,0,paradaER,indice,texto );
          listaMarcadores.push(markerNuevo);
        }   
        ubicacionAntLatLng=ubicacionLatLng;
        //console.log("SI");
...
function separacion(ubi1,ubi2,minDist){
    const distancia = google.maps.geometry.spherical.computeDistanceBetween(ubi1, ubi2);
    var bMayor = false;
...
function tiempoViaje(paradas,paraO,paraD){
  const service = new google.maps.DistanceMatrixService();
  var wps = paradas2WPS(paradas);
  var org = { lat: paraO["lat"], lng: paraO["lng"] };
  var dest = { lat: paraD["lat"], lng: paraD["lng"] };
  var request = {
    origins: [org],
  ...
  service.getDistanceMatrix(request, function(response, status) {
    ...
    var distanciaKM = response.rows[0].elements[0]['distance'].text;
    var distanciaVal = response.rows[0].elements[0]['distance'];
    var duracion = response.rows[0].elements[0]['duration'].text;
    //alert("Distacia: " + distanciaKM + ", Duracion: " + duracion);
    //var lbl1 = document.getElementById('lblDistancia');
    //var lbl2 = document.getElementById('lblTiempo');

    console.log(distanciaVal);
    var lbl = document.getElementById('labelInfo');
    lbl.innerHTML = "Distancia: " + distanciaKM + ", Duracion: " + duracion;
    //lbl.innerHTML += "Duracion: " + duracion;
    document.querySelector('.overlay').classList.add('active');
  });
}

function getUbicacion(marcador){
  
  var markerPosition = marcador.getPosition();
